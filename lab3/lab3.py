...
class Terminal(QMainWindow):
    def __init__(self, user):
        ...
        self.button_exit.setText("Вийти")
    ...
    def check_command(self):
        ...
        if input_command not in self.user.permitted_operations or "admin" in input_line:
            ...
            write_to_log(self.user.login, 'Користувачу відмовлено в доступі')
            check_oper_log(self.user.login)
        else:
            self.run(input_line)
    ...
    def exit(self):
        write_to_log(self.user.login, 'Користувач вийшов із системи')
        os.chmod(path, 0o775)
        sys.exit()
        ...
        for user in journal["users"]:
            ...
            if user["login"] == input_login and user["password"] == input_pass:
                if input_login == "admin":
                    write_to_log('admin', 'Користувач ввійшов в систему')
                    adm_w = AdminWindow()
                    adm_w.show()
                    self.close()
                    found = True
                    ...
                    write_to_log(cur_user.login, 'Користувач ввійшов в систему')
    ...
    def ok(self):
        ...
        if self.if_qst:
            if self.cur_user.answer != input_answ:
                ...
                write_to_log(self.cur_user.login, 'Користувач не пройшов аутентифікацію')
                self.check_errors()
                ent = EnterWindow()
                ent.show()
            else:
                self.close()
                write_to_log(self.cur_user.login, 'Користувач пройшов аутентифікацію')
...
class AdminWindow(QMainWindow):
    ...
    def closeEvent(self, event):
        write_to_log('admin', 'Користувач вийшов із системи')
        os.chmod(path, 0o775)
        sys.exit()
    ...
    def to_add_user(self):
        ...
        if check_passw(password, passw_len):
            ...
            if len(journal["users"]) - 1 >= users_number:
                ...
                write_to_log('admin', f'Адміністратор додав користувача {login}')
    ...
    def to_del_user(self):
        ...
        write_to_log('admin', f'Адміністратор видалив користувача {del_login}')
...
class ChangePass(QWidget):
    ...
    def ok(self):
        ...
        if inp_old_pass != self.user.password:
            ...
            write_to_log(self.cur_user.login, 'Користувач змінив пароль')
            with open(adm_path + '/journal.json', 'r') as file:
                journal = json.load(file)
...
def create_operations_log():
    df = pd.DataFrame(columns=["date", "user", "action"])
    df.to_csv(adm_path + '/operations_log.csv', index=False)


def write_to_log(user, action):
    df = pd.read_csv(adm_path + '/operations_log.csv')
    row = {"date": datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S'), "user": user, "action": action}
    df = df.append(row, ignore_index=True)
    df.to_csv(adm_path + '/operations_log.csv', index=False)


def check_oper_log(login):
    df = pd.read_csv(adm_path + '/operations_log.csv')
    count = df[(df["user"] == login) & (df["action"].str.contains('доступі'))].shape[0]
    if count >= permis_errors:
        with open(adm_path + '/journal.json', 'r') as file:
            journal = json.load(file)
        idx = 0
        while journal["users"][idx]["login"] != login:
            idx += 1
...
if __name__ == "__main__":
    ...
    permis_errors = 5   # кол-во допустимых отказов в доступе
    ...
    if not os.path.exists(adm_path + '/operations_log.csv'):
